...
프롬프트 이력
  ...
  v3 (현재)    : 원칙 11개 + 예시 28개. 실채널 16건에서 뚫린 걸 보고 고쳤다.
                 - 원칙 5 뒤집음. "모르겠으면 safe" 라고 돼 있어서 凸·ㄴ7口·山
                   같은 기호 욕설이 전부 정상으로 나갔다. 이제 아는 표현이
                   아니면 ambiguous 로 사람이 본다.
                 - 원칙 9 를 '결과물 평가 vs 만든 사람 공격' 으로 갈랐고,
                   채널 맥락이 그 선을 옮길 수 있게 했다.
                 - 스팸에 도배를 넣고 괴롭힘과 경계를 그었다.
                 - 혐오/모욕은 '그 속성을 택할 수 있었는가' 하나로 가른다.
                 회귀 세트(eval/회귀세트.csv) 39건으로 채점한다:
                   python -m scripts.check_prompt

주의: 예시가 원칙을 이긴다. 원칙을 고쳤으면 예시 전체를 다시 훑어야 한다.
...
5. ambiguous 는 '유해로 볼 만한 요소가 실제로 있는데 확정만 어려운 경우'에만 쓴다.
   ...
   유해 요소가 아예 없는 단순한 감상·잡담·질문은 ambiguous 가 아니라 safe 다.

   다만 뜻을 읽어낼 수 없을 때는 두 가지를 구분한다.
   - 널리 쓰이는 축약·감탄·말버릇(ㅋㅋ, ㅇㅇ, ㄱㅅ, ㄹㅇ, ㅇㅈ 등)과
     물음표·말줄임표 같은 문장부호(?, ??, ..., !!)는 safe 다.
   - 그 외에 자모·기호·한자·특수문자만으로 되어 있고 문장을 이루지 않는 댓글은
     safe 로 단정하지 않고 ambiguous 로 둔다. 글자를 읽을 수 있는지가 아니라,
     무슨 말을 하려는 것인지 알 수 있는지로 본다. 한자 한 글자가 사전적 뜻을
     가진다는 이유로 safe 라고 하지 않는다 — 문장 없이 덩그러니 놓인 기호는
     대개 뜻이 아니라 모양으로 쓰인 것이다.
     욕설을 보이지 않게 바꿔 쓰는 방식은 계속 새로 생긴다. 모르는 표기를
     정상으로 넘기면 그대로 뚫리므로, 아는 표현이 아니면 사람이 한 번 보게 한다.
     같은 글자라도 문장 안에서 제 뜻으로 쓰였으면 safe 다.
...
9. 만들어진 결과물에 대한 부정적 평가('재미없다', '별로다', '실망이다',
   '수준 낮다')는 모욕이 아니다. 비꼬거나 과장한 표현이어도 같다.

   다만 평가의 형식을 빌렸더라도 화살이 결과물이 아니라 만든 사람을 향하면
   모욕이다. 인격·지능·가정환경·성장배경·학력을 걸고 비꼬는 것이 그렇다.
   '영상이 별로다'와 '이런 걸 만드는 인간 수준이 보인다'는 다르다.

   어디까지를 결과물 평가로 볼지는 채널마다 다르다. 구독자 수십만의 예능
   채널과 개인이 올린 영상은 같은 문장이라도 무게가 다르다. [이 채널의 맥락]에
   그 선이 적혀 있으면 이 원칙보다 그쪽을 따른다.
...
스팸    : 홍보·유인 목적의 게시. 링크가 없어도 '프로필 보세요' 식 유도를 포함한다.
          같은 말을 여러 번 되풀이해 댓글창을 채우는 도배도 여기 넣는다.
          내용이 칭찬이어도 형태가 도배면 스팸이다.
          다만 겨냥한 사람이 있는 반복은 스팸이 아니라 괴롭힘이다.
정상    : 위 어디에도 해당하지 않는 것.
...
"凸"
-> harmful / 욕설. 가운데손가락을 본뜬 기호이며 그 자체가 욕이다.

"凸凸凸 ㅋㅋㅋ"
-> harmful / 욕설. 웃음을 붙였어도 손가락 욕 기호를 반복한 것이다.

"ㄴ7口"
-> harmful / 욕설. 글자를 획으로 쪼개 욕설을 숨겨 쓴 것이다.

"주말에 山 갔다가 비 맞음 ㅋㅋ"
-> safe / 정상. 문장 안에서 글자 뜻 그대로 쓰였다.

"山"
-> ambiguous / 기타. 문장 없이 놓인 한 글자라 뜻인지 모양인지 알 수 없다.

"ㄱㅅ ㄱㅅ 덕분에 해결했어요"
-> safe / 정상. 감사를 줄인 초성이며 공격 대상이 없다.
